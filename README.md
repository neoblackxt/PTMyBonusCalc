...
在用户详情页（`userdetails.php`）的当前做种列表中显示时魔/A 和 时魔/GB/A/GB 列（实验性功能，仅支持部分站点）。需要站点使用 `#ka1` 容器加载做种表格，且表格包含可识别的时间列、大小列和做种人数列。
...
脚本安装（Lite 版）：https://greasyfork.org/en/scripts/596872-pt站点魔力计算器-lite

**完整版说明**：完整版包含全部站点适配，但因其适配范围包含成人站点，GreasyFork 已将完整版标记为成人内容——匿名访问完整版页面会显示 404，需登录 GreasyFork 账号后才能查看完整版页面并安装：https://greasyfork.org/en/scripts/439369-pt站点魔力计算器

Lite 版与完整版功能逻辑完全相同，仅精简了极少量站点适配以满足 GreasyFork 公开列表的展示要求。
...
当前脚本已将 [PT监护室](https://savept.icu/) 中标记为"健康"的 139 个 PT 站点加入匹配规则，并同时覆盖主域名和子域名的 `torrents*` 页面。
